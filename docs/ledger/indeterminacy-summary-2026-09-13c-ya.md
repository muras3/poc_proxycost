# 断定不能（rankIndeterminate）台帳 — 要約 (2026-09-13c-ya)
...
- 品目数: 1 / 保管日数: 0 / site: yahoo-auctions
...
**closedByAssumption: 265/343（77.3%）** ——**`rankIndeterminate` とは独立の軸。**「断定不能ゼロ」は「全部確定」を意味しない: 1位行の `total.high` が閉じていて `rankIndeterminate: false` でも、それが検証済みの決定ではなく未確認の仮定（`Row.closedByAssumption`、PR #147）に依存していればここに数える。画面では「CHEAPEST」ではなく「ESTIMATED CHEAPEST」と表示される条件。
