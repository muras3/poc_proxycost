...
|---|---|---|---|---|:---:|---|
| `jauce/2026-09-12-six-countries-v2.json` | `05bea962-jauce_shipping_measurements_v2.json` | 2026-09-12 | US/GB/FR/AU/CA/SG、214観測/140 not_available。**取り込まれた形跡なし**(`master/courier-rates.json` の `jauce_six_countries_v2_2026_09_12` の `capture_file` がこのファイルを指す)| PR #79 | ✓ | 🔴 同日・同一問題(SUBMIT残留表示バグ、タブ間汚染)を抱える回であり、下記 `2c1862f3` 系列と合わせて全体が疑わしい。個別に再測定されていない値は信用しない。 |
...
ディレクトリ合計 約3.9MB(29件のアップロードのうち、上記の完全重複1件を除いた28件)。
単体で突出して大きいファイルは無い(最大は約720KB)。
