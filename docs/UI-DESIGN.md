...
- 差額は主に公表料金の差から生じ、総額（重量の推定が効く）より確か。**「重量を 1/3〜5倍に外しても7カ国で順位は動かない」と以前ここに書いていた根拠は 2026-09-06 の再測定で否定された**（1位が入れ替わる重量は2点 1,150g／3点 1,325g／5点 1,625g。DESIGN-NOTES §1）ので、順位が動く条件では `rankStable` をそのまま出す。それでも最も確かな情報を最も大きく、最も不確かな情報を最も小さく、という並べ方は変えない。
...
| 総額を桁まで表示（`¥33,528`） | 確定値と同じ見た目になる。**総額の 16%（推定 14% ＋ 二次情報 2%）は推論**という事実と矛盾（2026-09-07 実測、DESIGN-NOTES §1。却下した当時は「約半分が推論」だった） |
