...
マスタは目視ではなく機械で検証する。**現在すべて通過している。**

```
== 1. スキーマ ==
  費目行 66 / catalog 39 / 国 9 / 通関経路 21

== 2. 実請求の再現 ==
  [PASS] es-zenmarket-ups-2023-07-26   一部が循環   手数料 15.89/15.90  合計 342.51/342.52
  [PASS] ca-canadapost-forum           独立         GST 99.44  PST 139.21  合計 248.59/248.60
  [PASS] es-fedex-30pct                循環         手数料 7.54/7.54
  [PASS] es-correos-selfclear          独立         IVA 26.25  手数料VAT 0.27  合計 27.81/27.81
  [SKIP] au-zenmarket-ems              再現不可

OK: スキーマ検証を通過 / 実請求 4/5 件を再現（うち独立検証 2 件）
```
