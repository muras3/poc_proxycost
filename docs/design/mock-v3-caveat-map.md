...
| 7 | rank unstable with weight | form (`!` line) + link to the decisive item | which title to fix has no shape |
...
| 82 | "goods restrictions not checked" | form-adjacent: 1 line, no `!` unless GB/DE (lithium) | kill-list §9: a mark on every screen is not a warning, it's wallpaper |
