 ...
 * このPRで足す器。このコメントを書いた当時（このPR時点）はどの社にもデータを
 * 入れておらず、並行作業者が `master/courier-rates.json` を取っている段階
 * （`docs/audit/o2-courier-2026-09-08.md` の便ID一覧に対応）だった。
 * **2026-09-13 時点ではもう違う。** PR #87（コミット `91b9532`）で `services.ts`
 * の16の宅配便ブロックに実データが入っており、この型の値は実際に選ばれる
 * （国ごとのカバレッジの正確な内訳は `services.ts` の `weightPointsByCountry`
 * を実際に数えて確認すること）。
